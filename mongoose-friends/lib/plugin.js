...
var Friendship = {
  ...
  status: {
    type: String,
    enum: Object.keys(Status).map(function (key) {
      return Status[key]
    })
  },
...
function plugin(options) {
  ...
  var friendshipBetween = function (m1, m2) {
    ...
    return function (done) {
      this.findOne({user: m1}, proj, function (err, doc) {
  ...
  return function friends(schema) {
      ...
      resolveArgs(arguments, function (conditions, fields, options, cb) {
        ...
        async.parallel({
          ...
          locals: function (done) {
            var select = {};
            select[pathName] = 1;
            Model.findOne({user: model}, select, function (err, doc) {
              if (err) return done(err);
          ...
          for (var i = 0, friendship; i < res.friends.length; i++) {
            friendship = res.locals[res.friends[i].user];
    ...
    schema.statics.removeFriend = function (m1, m2, cb) {
      ...
      var pull = function (m1, m2) {
        ...
        return function (done) {
          collection.update({user: m1}, update, done);
        }
      }
